...
void studentHome(student_c* student) {
	...
	while (signedIn) {
		...
		std::cout << "4: Display GPA\n";
		std::cout << "5: Display all student info\n";
		std::cout << "0: Sign out\n";
		std::cin >> choice;
		std::cout << std::endl;
		...
		switch (choice) {
			...
			case 2: { student->addDropCourses(); break; }
			case 3: { student->printSchedule(); break; }
			case 4: { student->printGPA(); break; }
			case 5: { student->printAll(); break; }
			case 0: {
				do {
...
void instructorHome(instructor_c* instructor) {
	...
	while (signedIn) {
		...
		std::cout << "3: Display classlist\n";
		std::cout << "4: Display all instructor info\n";
		std::cout << "0: Sign out\n";
		std::cin >> choice;
		std::cout << std::endl;
		...
		switch (choice) {
			...
			case 2: { instructor->printSchedule(); break; }
			case 3: { instructor->printClasslist(); break; }
			case 4: { instructor->printAll(); break; }
			case 0: {
				do {
					if (signedIn != 0 && signedIn != 1)
						std::cout << "Please enter a valid input.\n\n";
